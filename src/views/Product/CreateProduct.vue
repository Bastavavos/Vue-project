...
export default {
  ...
  computed: {
    currentUserData() {
      return usehttpStore().currentUser;
    }
  },
    methods: {
      async createMyProduct() {
        try {
          ...
          const productData = {
            ...
            artisan: this.currentUserData.firstname + ' ' + this.currentUserData.lastname,
          };

          console.log('Product Data:', productData);

          try {
            await store.createProduct(productData, this.token);
    ...
    created() {
      ...
      this.currentUser = usehttpStore().currentUser;
    }
}
</script>
        ...
        <div class="flex flex-col justify-center">
          ...
          <div class="flex flex-row space-x-14 justify-center">
            ...
            <div class="flex flex-col space-y-6">
              ...
              <input class="border rounded p-2 mb-4" v-model="price" type="number" placeholder="Product price"
                     required :class="{'input-bg-light': !isDarkTheme, 'input-bg-dark': isDarkTheme}">
              <input class="border rounded p-2 mb-4" v-model="stock" type="number" placeholder="Product stock"
                     required :class="{'input-bg-light': !isDarkTheme, 'input-bg-dark': isDarkTheme}">
          ...
          <div class="flex justify-center mt-10">
            <button @click="createMyProduct" type="button" class="btn text-white">Create</button>
          </div>
        </div>
    </div>
  </div>
